...
const ModalUpdateUser = ({ id }: IModal): JSX.Element => {
  ...
  const update = async () => {
    try {
      ...
      if (password != Password) {
        await fetch(
          `https://Kyrsa4back.ev1lg0.repl.co/api/users/password/${userId}`,
          {
            method: 'PUT',
            mode: 'cors',
            headers: {
  ...
  return (
    <ModalCard id={id} header={'Изменить данные'}>
      <FormLayout>
        <FormItem
          top="ФИО"
        ...
        <FormItem
          top="Пароль"
          bottom={!Password && 'Введите пароль'}
        ...
        <FormItem>
          <Button
            ...
            stretched
            onClick={update}
            disabled={PassportId.length === 0 || Adress.length === 0}
          >
            Обновить
          </Button>
        </FormItem>
